...
import {
  ...
  polygon,
  sepolia
} from "./chunk-XEC5VTGH.js";
import {
  validate
} from "./chunk-P6JGHPC7.js";
import {
  createPublicClient,
  getContract,
  http,
...
import "./chunk-PXQ245YX.js";
import "./chunk-GQ2UFVY6.js";
import {
  toFunctionSelector
...
// node_modules/@web3-name-sdk/core/dist/index.js
...
// node_modules/@web3-name-sdk/core/node_modules/@adraffy/ens-normalize/dist/index.mjs
...
function read_deltas2(n, next) {
  let v = Array(n);
  for (let i = 0, x = 0; i < n; i++) v[i] = x += signed2(next());
  return v;
}
...
function compose_pair2(a, b) {
  if (a >= L02 && a < L12 && b >= V02 && b < V12) {
    return S02 + (a - L02) * N_COUNT2 + (b - V02) * T_COUNT2;
  } else if (is_hangul2(a) && b > T02 && b < T12 && (a - S02) % T_COUNT2 == 0) {
    return a + (b - T02);
  } else {
...
function init2() {
  ...
  GROUPS2 = read_array_while2((i) => {
    let N = read_array_while2(r).map((x) => x + 96);
    if (N.length) {
      let R = i >= unrestricted;
      N[0] -= 32;
      N = str_from_cps2(N);
  ...
  let wholes = read_sorted_array().concat(Array_from2(WHOLE_VALID2)).sort((a, b) => a - b);
  wholes.forEach((cp, i) => {
    let d = r();
    let w = wholes[i] = d ? wholes[i - d] : { V: [], M: /* @__PURE__ */ new Map() };
    w.V.push(cp);
...
function check_fenced2(cps) {
  ...
  if (last == n) throw error_placement2(`trailing ${prev}`);
}
function safe_str_from_cps2(cps, max = Infinity, quoter = quote_cp2) {
  ...
  buf.push(str_from_cps2(cps.slice(prev, n)));
  return buf.join("");
}
...
function flatten2(split3) {
  return split3.map(({ input, error, output }) => {
    if (error) {
      let msg = error.message;
      throw new Error(split3.length == 1 ? msg : `Invalid label ${bidi_qq2(safe_str_from_cps2(input, 63))}: ${msg}`);
    }
...
var normalize2 = (name) => name ? ens_normalize2(name) : name;
...
function validateName(name) {
  ...
  const normalizedArray = labelArr.map((label) => {
    return isEncodedLabelhash(label) ? label : normalize2(label);
  });
  try {
...
function validateLabelLength(name, allowShortLabel = false) {
  ...
  let normalizedValue;
  try {
    normalizedValue = normalize2(name);
  } catch (e) {
...
var Web3Name = class {
  ...
  async getAddress(name, { coinType, rpcUrl } = {}) {
    ...
    const normalizedDomain = "lens" === tld ? name : normalize(name);
  ...
  async getContentHash({ name, rpcUrl }) {
    ...
    try {
      ...
      if (!tldInfo)
        throw "TLD not found";
      const namehash5 = tldNamehash(normalize(name), isV2Tld(tld) ? void 0 : tldInfo.identifier);
...
//# sourceMappingURL=@web3-name-sdk_core.js.map
